...
outputs <- foreach(i = 1:bs_iterations, .combine = "rbind", .packages = c("tidyverse", "nnet", "SuperLearner")) %dopar% {
  ...
  estimates <- decompose_dr_xfit_intA(data, formula_treatment, preds_outcome, sl_models)
...
# save results
save.image("/home/sozialwiss/einhofja/Dokumente/analysis_decomp/output_total_a_em.RData")
