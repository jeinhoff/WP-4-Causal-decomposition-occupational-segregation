...
decompose_uncond_dr_detailed <- function(data, formula_treatment, preds_outcome, sl_models) {
  
  # select complete cases
  data <- data %>% na.omit()
  
  # determine counter-factual composition via intervention rule: equalisation by education
  ...
  colnames(data_male)[52:79] = paste0("prop_occ_", colnames(data_male)[52:79])
  
  data_female <- cbind(data_female, as.data.frame(predict(model_female, newdata = data_female, type = "probs")))
  colnames(data_female)[52:79] = paste0("prop_occ_", colnames(data_female)[52:79])
  ...
  # predictions
  for (i in sort(unique(data$eseg_class_2))) {
    
    data_male$eseg_class_2 <- i
    data_female$eseg_class_2 <- i
  ...
  cf_prob_fc_occ <- data %>% 
    ...
    pivot_longer(cols = c(4:31), names_to = "occupation") %>% 
  ...
  weight <- data %>% 
    ...
    left_join(inv_fc_prob_fc_occ, by = c("orgpid", "age")) %>%
    mutate(combined_weight = weight * cf_prob_fc_occ * inv_fc_prob_fc_occ) %>%
    select(orgpid, age, combined_weight) %>%
    group_by(orgpid, age) %>% slice(1)
  
  # join
  data <- data %>% left_join(exp_outcome_fc_occ, by = c("orgpid", "age")) %>% left_join(weight, by = c("orgpid", "age"))
  
  # calculate bias correction
  bias_correction_m <- data %>% filter(female == 0) %>% 
    summarise(female = female, bias_correction = weighted.mean(outcome - exp_outcome_fc_occ, w = combined_weight)) %>% unique()
  
  bias_correction_f <- data %>% filter(female == 1) %>% 
    summarise(female = female, bias_correction = weighted.mean(outcome - exp_outcome_fc_occ, w = combined_weight)) %>% unique()
  
  bias_correction <- rbind(bias_correction_m, bias_correction_f)
  
  rm(cf_prob_fc_occ, inv_fc_prob_fc_occ, exp_outcome_fc_occ, weight, bias_correction_m, bias_correction_f)
  
  # correct bias in outcome predictions
  data <- data %>% 
    left_join(bias_correction, by = "female") %>% 
  ...
  # post-intervention
  levels_post_female <- data %>% filter(female == 1) %>% mutate(exp = (exp_outcome_occ_11 * counterfact_share_occ_11) + (exp_outcome_occ_12 * counterfact_share_occ_12) + (exp_outcome_occ_13 * counterfact_share_occ_13) + (exp_outcome_occ_21 * counterfact_share_occ_21) + (exp_outcome_occ_22 * counterfact_share_occ_22) + (exp_outcome_occ_23 * counterfact_share_occ_23) + (exp_outcome_occ_24 * counterfact_share_occ_24) + (exp_outcome_occ_25 * counterfact_share_occ_25) + (exp_outcome_occ_31 * counterfact_share_occ_31) + (exp_outcome_occ_32 * counterfact_share_occ_32) + (exp_outcome_occ_33 * counterfact_share_occ_33) + (exp_outcome_occ_34 * counterfact_share_occ_34) + (exp_outcome_occ_41 * counterfact_share_occ_41) + (exp_outcome_occ_42 * counterfact_share_occ_42) + (exp_outcome_occ_43 * counterfact_share_occ_43) + (exp_outcome_occ_51 * counterfact_share_occ_51) + (exp_outcome_occ_52 * counterfact_share_occ_52) + (exp_outcome_occ_53 * counterfact_share_occ_53) + (exp_outcome_occ_54 * counterfact_share_occ_54) + (exp_outcome_occ_61 * counterfact_share_occ_61) + (exp_outcome_occ_62 * counterfact_share_occ_62) + (exp_outcome_occ_63 * counterfact_share_occ_63) + (exp_outcome_occ_64 * counterfact_share_occ_64) + (exp_outcome_occ_65 * counterfact_share_occ_65) + (exp_outcome_occ_71 * counterfact_share_occ_71) + (exp_outcome_occ_72 * counterfact_share_occ_72) + (exp_outcome_occ_73 * counterfact_share_occ_73) + (exp_outcome_occ_74 * counterfact_share_occ_74)) %>% summarize(weighted.mean(exp, w = weight) * 100) %>% as.numeric()
  levels_post_male <- data %>% filter(female == 0) %>% mutate(exp = (exp_outcome_occ_11 * counterfact_share_occ_11) + (exp_outcome_occ_12 * counterfact_share_occ_12) + (exp_outcome_occ_13 * counterfact_share_occ_13) + (exp_outcome_occ_21 * counterfact_share_occ_21) + (exp_outcome_occ_22 * counterfact_share_occ_22) + (exp_outcome_occ_23 * counterfact_share_occ_23) + (exp_outcome_occ_24 * counterfact_share_occ_24) + (exp_outcome_occ_25 * counterfact_share_occ_25) + (exp_outcome_occ_31 * counterfact_share_occ_31) + (exp_outcome_occ_32 * counterfact_share_occ_32) + (exp_outcome_occ_33 * counterfact_share_occ_33) + (exp_outcome_occ_34 * counterfact_share_occ_34) + (exp_outcome_occ_41 * counterfact_share_occ_41) + (exp_outcome_occ_42 * counterfact_share_occ_42) + (exp_outcome_occ_43 * counterfact_share_occ_43) + (exp_outcome_occ_51 * counterfact_share_occ_51) + (exp_outcome_occ_52 * counterfact_share_occ_52) + (exp_outcome_occ_53 * counterfact_share_occ_53) + (exp_outcome_occ_54 * counterfact_share_occ_54) + (exp_outcome_occ_61 * counterfact_share_occ_61) + (exp_outcome_occ_62 * counterfact_share_occ_62) + (exp_outcome_occ_63 * counterfact_share_occ_63) + (exp_outcome_occ_64 * counterfact_share_occ_64) + (exp_outcome_occ_65 * counterfact_share_occ_65) + (exp_outcome_occ_71 * counterfact_share_occ_71) + (exp_outcome_occ_72 * counterfact_share_occ_72) + (exp_outcome_occ_73 * counterfact_share_occ_73) + (exp_outcome_occ_74 * counterfact_share_occ_74)) %>% summarize(weighted.mean(exp, w = weight) * 100) %>% as.numeric()
  
  rm(data)
  
  # calculate estimands
  gap_pre <- abs(levels_pre_female - levels_pre_male)
  
  gap_pre <- ifelse(levels_pre_female < levels_pre_male, gap_pre, gap_pre * (-1))
  
  gap_post <- abs(levels_post_female - levels_post_male)
  
  gap_post <- ifelse(levels_post_female < levels_post_male, gap_post, gap_post * (-1))
  
  change <- abs(gap_post - gap_pre)
  
  change <- ifelse(abs(gap_pre) > abs(gap_post), change * (-1), change)
  
  change_pp <- abs(change / gap_pre) * 100
  
  change_pp <- ifelse(change < 0, change_pp * (-1), change_pp)
...
decompose_cond_dr_xfit <- function(data, formula_treatment, preds_outcome, sl_models) {
...
decompose_cond_dr <- function(data, formula_treatment, preds_outcome, sl_models) {
  ...
  # treatment model
  data_male <- data %>% filter(female == 0) %>% ungroup()
  data_female <- data %>% filter(female == 1) %>% ungroup()
  ...
  data <- data %>% as.data.frame() %>% ungroup()
  ...
  # train
  model_male <- SuperLearner(Y = data_male$outcome, X = data_male %>% select(all_of(preds_outcome)), SL.library = sl_models, family = binomial(), obsWeights = data_male$weight, cvControl = list(V = 0))
  model_female <- SuperLearner(Y = data_female$outcome, X = data_female %>% select(all_of(preds_outcome)), SL.library = sl_models, family = binomial(), obsWeights = data_female$weight, cvControl = list(V = 0))
  ...
  inv_fc_prob_fc_occ <- data %>% 
    ...
    select(orgpid, age, inv_fc_prob_fc_occ) %>%
    group_by(orgpid, age) %>% slice(1) %>% ungroup() %>%
    mutate(inv_fc_prob_fc_occ = case_when(inv_fc_prob_fc_occ > quantile(inv_fc_prob_fc_occ, 0.99) ~ quantile(inv_fc_prob_fc_occ, 0.99),
                                          inv_fc_prob_fc_occ < quantile(inv_fc_prob_fc_occ, 0.01) ~ quantile(inv_fc_prob_fc_occ, 0.01),
                                          T ~ inv_fc_prob_fc_occ))
  ...
  # join
  data <- data %>% left_join(exp_outcome_fc_occ, by = c("orgpid", "age")) %>% left_join(weight, by = c("orgpid", "age")) %>% ungroup()
  ...
  rm(list = setdiff(ls(), c("estimates", "data_final", "decompose_cond_dr", "formula_treatment", "preds_outcome", "sl_models")))
...
decompose_cond <- function(data, formula_treatment, preds_outcome, sl_models) {
  ...
  for (i in sort(unique(data$eseg_class))) {
    ...
    data_male$eseg_class <- i
    data_female$eseg_class <- i
...
decompose_uncond_dr_xfit <- function(data, formula_treatment, preds_outcome, sl_models) {
...
split <- sample(nrow(data), size = ceiling(nrow(data) / 2), replace = F)
...
# treatment model
data_split_1_male <- data_split_1 %>% filter(female == 0)
data_split_2_male <- data_split_2 %>% filter(female == 0)
data_split_1_female <- data_split_1 %>% filter(female == 1)
data_split_2_female <- data_split_2 %>% filter(female == 1)
...
# predict using model trained on other split
# men
data_split_1_male <- cbind(data_split_1_male, as.data.frame(predict(model_male_split_2, newdata = data_split_1_male, type = "probs")))
colnames(data_split_1_male)[32:38] = paste0("prop_occ_", colnames(data_split_1_male)[32:38])

data_split_2_male <- cbind(data_split_2_male, as.data.frame(predict(model_male_split_1, newdata = data_split_2_male, type = "probs")))
colnames(data_split_2_male)[32:38] = paste0("prop_occ_", colnames(data_split_2_male)[32:38])

# women
data_split_1_female <- cbind(data_split_1_female, as.data.frame(predict(model_male_split_2, newdata = data_split_1_female, type = "probs")))
colnames(data_split_1_female)[32:38] = paste0("prop_occ_", colnames(data_split_1_female)[32:38])

data_split_2_female <- cbind(data_split_2_female, as.data.frame(predict(model_male_split_1, newdata = data_split_2_female, type = "probs")))
colnames(data_split_2_female)[32:38] = paste0("prop_occ_", colnames(data_split_2_female)[32:38])

# combine
data_split_1 <- rbind(data_split_1_male, data_split_1_female)
data_split_2 <- rbind(data_split_2_male, data_split_2_female)

rm(data_split_1_male, data_split_1_female, data_split_2_male, data_split_2_female, model_male_split_1, model_male_split_2, model_female_split_1, model_female_split_2)
...
# train
# men
model_male_split_1 <- SuperLearner(Y = data_split_1_male$outcome, X = data_split_1_male %>% select(all_of(preds_outcome)), SL.library = sl_models, family = binomial(), obsWeights = data_split_1_male$weight)
model_male_split_2 <- SuperLearner(Y = data_split_2_male$outcome, X = data_split_2_male %>% select(all_of(preds_outcome)), SL.library = sl_models, family = binomial(), obsWeights = data_split_2_male$weight)

# women
model_female_split_1 <- SuperLearner(Y = data_split_1_female$outcome, X = data_split_1_female %>% select(all_of(preds_outcome)), SL.library = sl_models, family = binomial(), obsWeights = data_split_1_female$weight)
model_female_split_2 <- SuperLearner(Y = data_split_2_female$outcome, X = data_split_2_female %>% select(all_of(preds_outcome)), SL.library = sl_models, family = binomial(), obsWeights = data_split_2_female$weight)

# predictions
for (i in sort(unique(data$eseg_class))) {
  
  data_split_1_male$eseg_class <- i
  data_split_2_male$eseg_class <- i
  data_split_1_female$eseg_class <- i
  data_split_2_female$eseg_class <- i
  
  data_split_1_male$prediction <- as.data.frame(predict(model_male_split_2, newdata = data_split_1_male, type = "response"))$pred
  data_split_2_male$prediction <- as.data.frame(predict(model_male_split_1, newdata = data_split_2_male, type = "response"))$pred
  data_split_1_female$prediction <- as.data.frame(predict(model_female_split_2, newdata = data_split_1_female, type = "response"))$pred
  data_split_2_female$prediction <- as.data.frame(predict(model_female_split_1, newdata = data_split_2_female, type = "response"))$pred
  
  data_split_1_male <- data_split_1_male %>% rename_with(~paste0("exp_outcome_occ_", i), .cols = prediction)
  data_split_2_male <- data_split_2_male %>% rename_with(~paste0("exp_outcome_occ_", i), .cols = prediction)
  data_split_1_female <- data_split_1_female %>% rename_with(~paste0("exp_outcome_occ_", i), .cols = prediction)
  data_split_2_female <- data_split_2_female %>% rename_with(~paste0("exp_outcome_occ_", i), .cols = prediction)
  
}

# combine
preds_split_1 <- rbind(data_split_1_male, data_split_1_female)
preds_split_2 <- rbind(data_split_2_male, data_split_2_female)

preds_split_1 <- preds_split_1 %>% select(orgpid, age, contains("exp_outcome_occ_")) %>% group_by(orgpid, age) %>% slice(1)
preds_split_2 <- preds_split_2 %>% select(orgpid, age, contains("exp_outcome_occ_")) %>% group_by(orgpid, age) %>% slice(1)

data_split_1 <- data_split_1 %>% left_join(preds_split_1, by = c("orgpid", "age"))
data_split_2 <- data_split_2 %>% left_join(preds_split_2, by = c("orgpid", "age"))

rm(i, data_split_1_male, data_split_1_female, data_split_2_male, data_split_2_female, model_male_split_1, model_male_split_2, model_female_split_1, model_female_split_2, preds_split_1, preds_split_2)
...
# inverse of the factual probability of the factual occupation
inv_fc_prob_fc_occ_split_1 <- data_split_1 %>% 
  select(orgpid, age, eseg_class, contains("prop_occ")) %>%
  pivot_longer(cols = c(4:10), names_to = "occupation") %>% 
  mutate(occupation = substr(occupation, 10, 10)) %>% 
  filter(eseg_class == occupation) %>%
...
inv_fc_prob_fc_occ_split_2 <- data_split_2 %>% 
  select(orgpid, age, eseg_class, contains("prop_occ")) %>%
  pivot_longer(cols = c(4:10), names_to = "occupation") %>% 
  mutate(occupation = substr(occupation, 10, 10)) %>% 
  filter(eseg_class == occupation) %>%
...
# outcome predictions
exp_outcome_fc_occ_split_1 <- data_split_1 %>%
  select(orgpid, age, eseg_class, contains("exp_outcome_occ")) %>%
  pivot_longer(cols = c(4:10), names_to = "occupation") %>% 
  mutate(occupation = substr(occupation, 17, 17)) %>% 
  filter(eseg_class == occupation) %>%
...
exp_outcome_fc_occ_split_2 <- data_split_2 %>%
  select(orgpid, age, eseg_class, contains("exp_outcome_occ")) %>%
  pivot_longer(cols = c(4:10), names_to = "occupation") %>% 
  mutate(occupation = substr(occupation, 17, 17)) %>% 
  filter(eseg_class == occupation) %>%
...
# join
data_split_1 <- data_split_1 %>% left_join(exp_outcome_fc_occ_split_1, by = c("orgpid", "age")) %>% left_join(weight_split_1, by = c("orgpid", "age"))
data_split_2 <- data_split_2 %>% left_join(exp_outcome_fc_occ_split_2, by = c("orgpid", "age")) %>% left_join(weight_split_2, by = c("orgpid", "age"))

# calculate bias correction
bias_correction_m_split_1 <- data_split_1 %>% filter(female == 0) %>% 
  summarise(female = female, bias_correction = weighted.mean(outcome - exp_outcome_fc_occ, w = combined_weight)) %>% unique()

bias_correction_m_split_2 <- data_split_2 %>% filter(female == 0) %>% 
  summarise(female = female, bias_correction = weighted.mean(outcome - exp_outcome_fc_occ, w = combined_weight)) %>% unique()

bias_correction_f_split_1 <- data_split_1 %>% filter(female == 1) %>% 
  summarise(female = female, bias_correction = weighted.mean(outcome - exp_outcome_fc_occ, w = combined_weight)) %>% unique()

bias_correction_f_split_2 <- data_split_2 %>% filter(female == 1) %>% 
  summarise(female = female, bias_correction = weighted.mean(outcome - exp_outcome_fc_occ, w = combined_weight)) %>% unique()
...
data_split_1 <- data_split_1 %>% 
  left_join(bias_correction_split_1, by = "female") %>% 
  mutate_at(vars(contains("exp_outcome_occ_")), ~(. - bias_correction)) %>% 
  mutate_at(vars(contains("exp_outcome_occ_")), ~(case_when(. > 1 ~ 1, . < 0 ~ 0, T ~ .)))

data_split_2 <- data_split_2 %>% 
  left_join(bias_correction_split_2, by = "female") %>% 
  mutate_at(vars(contains("exp_outcome_occ_")), ~(. - bias_correction)) %>% 
  mutate_at(vars(contains("exp_outcome_occ_")), ~(case_when(. > 1 ~ 1, . < 0 ~ 0, T ~ .)))
...
# post-intervention
# split 1
levels_post_female_split_1 <- data_split_1 %>% filter(female == 1) %>% mutate(exp = (exp_outcome_occ_1 * counterfact_share_occ_1) + (exp_outcome_occ_2 * counterfact_share_occ_2) + (exp_outcome_occ_3 * counterfact_share_occ_3) + (exp_outcome_occ_4 * counterfact_share_occ_4) + (exp_outcome_occ_5 * counterfact_share_occ_5) + (exp_outcome_occ_6 * counterfact_share_occ_6) + (exp_outcome_occ_7 * counterfact_share_occ_7)) %>% summarize(weighted.mean(exp, w = weight) * 100) %>% as.numeric()

levels_post_male_split_1 <- data_split_1 %>% filter(female == 0) %>% mutate(exp = (exp_outcome_occ_1 * counterfact_share_occ_1) + (exp_outcome_occ_2 * counterfact_share_occ_2) + (exp_outcome_occ_3 * counterfact_share_occ_3) + (exp_outcome_occ_4 * counterfact_share_occ_4) + (exp_outcome_occ_5 * counterfact_share_occ_5) + (exp_outcome_occ_6 * counterfact_share_occ_6) + (exp_outcome_occ_7 * counterfact_share_occ_7)) %>% summarize(weighted.mean(exp, w = weight) * 100) %>% as.numeric()

# split 2
levels_post_female_split_2 <- data_split_2 %>% filter(female == 1) %>% mutate(exp = (exp_outcome_occ_1 * counterfact_share_occ_1) + (exp_outcome_occ_2 * counterfact_share_occ_2) + (exp_outcome_occ_3 * counterfact_share_occ_3) + (exp_outcome_occ_4 * counterfact_share_occ_4) + (exp_outcome_occ_5 * counterfact_share_occ_5) + (exp_outcome_occ_6 * counterfact_share_occ_6) + (exp_outcome_occ_7 * counterfact_share_occ_7)) %>% summarize(weighted.mean(exp, w = weight) * 100) %>% as.numeric()

levels_post_male_split_2 <- data_split_2 %>% filter(female == 0) %>% mutate(exp = (exp_outcome_occ_1 * counterfact_share_occ_1) + (exp_outcome_occ_2 * counterfact_share_occ_2) + (exp_outcome_occ_3 * counterfact_share_occ_3) + (exp_outcome_occ_4 * counterfact_share_occ_4) + (exp_outcome_occ_5 * counterfact_share_occ_5) + (exp_outcome_occ_6 * counterfact_share_occ_6) + (exp_outcome_occ_7 * counterfact_share_occ_7)) %>% summarize(weighted.mean(exp, w = weight) * 100) %>% as.numeric()
...
rm(levels_post_female_split_1, levels_post_female_split_2, levels_post_male_split_1, levels_post_male_split_2)
...
decompose_uncond_dr <- function(data, formula_treatment, preds_outcome, sl_models) {
...
decompose_uncond <- function(data, formula_treatment, preds_outcome, sl_models) {
...
get_results <- function(decomp_results) {
  ...
  results <- decomp_results %>%
    ...
    summarise(
      se = sd(est),
      est = mean(est),
      conf_lower_999 = est - 3.2905 * se, conf_higher_999 = est + 3.2905 * se,
      conf_lower_99 = est - 2.5758 * se, conf_higher_99 = est + 2.5758 * se,
      conf_lower_95 = est - 1.9600 * se, conf_higher_95 = est + 1.9600 * se,
      sig = case_when(
        ...
        T ~ "")) %>%
    select(estimand, est, se, sig, conf_lower_999, conf_higher_999, conf_lower_99, conf_higher_99, conf_lower_95, conf_higher_95)
